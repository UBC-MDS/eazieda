...
def test_missing_impute(df_miss):
    ...
    expected_output_default = pd.DataFrame(data = {'a': [1., 2., 2., 3.], 'b': ['x', 'y', 'y', 'y']}).reset_index(drop=True)
    ...
    expected_output_two_drop = pd.DataFrame(data = {'a': [1., 3.], 'b': ['x', 'y']}).reset_index(drop=True)
    
    missing_output_two_drop = missing_impute(df_miss, method_num="drop", method_non_num="drop")
    
    print(expected_output_two_drop)
    print(missing_output_two_drop)

    assert pd.DataFrame.equals(missing_output_two_drop, expected_output_two_drop)
    
    
    # Test with only one drop argument is selected
    expected_output_one_drop = pd.DataFrame(data = {'a': [1., 2., 3.], 'b': ['x', 'y', 'y']}).reset_index(drop=True)
    ...
    expected_output_median = pd.DataFrame(data = {'a': [1., 2., 2., 3.], 'b': ['x', 'y', 'y', 'y']}).reset_index(drop=True)
